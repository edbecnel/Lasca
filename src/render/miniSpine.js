...
/**
 * Draws the compact mini-stack spine next to the top piece.
 * - Shows actual mini pieces (up to maxShown).
 * - If the stack is taller than maxShown, shows bottom half + top half and draws a crack between them.
 */
export function drawMiniStackSpine(svgRoot, g, cx, cy, stack, opts = {}){
  const {
    pieceSize = 86,
  ...
  // Decide which pieces to show in the mini-spine (bottom->top order).
  let shown = [];
  let hasCrack = false;
  ...
  // Clip minis to the pill (definitions must live in <defs>)
  ...
  const minis = document.createElementNS(SVG_NS, "g");
  ...
  // crack position is between bottom and top halves
  ...
  // Draw the "crack" between the halves (only when n > maxShown).
  if (hasCrack){
